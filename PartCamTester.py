...
def main():
    ...
    for row in range(2, 47604):
        ...
        dic['score'] = ws['W' + str(row)].value
        whole_dict.append(dic)
    with open('whole_collection_processed.txt','w')as f:
        f.write(json.dumps(whole_dict, indent=4))

    '''
    wb = load_workbook('whole_collection_processed.xlsx')
    ws = wb['Sheet1']
    for row in range(2, 22308):
        dic = {}
        dic['kit_name'] = ws['A' + str(row)].value
        dic['plate_name'] = ws['B' + str(row)].value
        dic['plate_well'] = ws['D' + str(row)].value
        dic['part_plasmid'] = ws['E' + str(row)].value
        dic['part_id'] = ws['F' + str(row)].value
        dic['part_cell'] = ws['G' + str(row)].value
        dic['part_comments'] = ws['H' + str(row)].value
        whole_dict.append(dic)
    with open('distribution_tiks.txt','w')as f:
        f.write(json.dumps(whole_dict, indent=4))
        '''
